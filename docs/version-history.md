...
- **v0.4.44** — Two-zone header for `mote show` first line and every `mote ls` row: status icon + Accent-colored ID + ` · ` + title on the left, `[<icon> <STATUS> w<weight>]` right-aligned to terminal width on the right. Closed motes (completed / archived / deprecated) mute the entire line and drop the weight segment — triage is done, the signal is no longer load-bearing. Long titles truncate with `…` (pretty) or `...` (plain); the right zone is never truncated. `--plain` mode emits the same content with ASCII icons and a literal `  |  ` separator, no padding, no ANSI — LLM- and `awk -F'|'`-friendly. `--json` is byte-for-byte unchanged. New `format.RenderHeader` in `internal/format/header.go` (sibling `TerminalWidth` honors the test-only `MOTE_FORCE_WIDTH` env override). The old printf-table column header on `mote ls` and the `=== <id> ===` block + four redundant `key: value` lines (id/status/title/weight) on `mote show` are gone; `type` and the rest of the body fields stay. `docs/UI_PHILOSOPHY.md` gains a "Header structure" section between Rules 2 and 3. Scope guard test pins `mote pulse` / `mote constellation` / `mote context` away from the new helper. (STORY-HDRZ-001)
- **v0.4.43** — Six semantic color tokens (`Pass`, `Warn`, `Fail`, `Accent`, `Muted`, `Command`) defined in `internal/format/style.go`, each backed by an `AdaptiveColor{Light, Dark}` pair sourced from the Ayu palette (Pass/Warn/Fail verbatim from beads-recommendations §23.13; Accent/Command picked from Ayu Sublime / VS Code for tonal match). `COLORFGBG`-based light/dark detection is hand-rolled — no new dep. `Muted` keeps its `\x1b[2m` SGR-dim implementation byte-for-byte (preserving STORY-MUTED-001's snapshot tests); `Pass`/`Warn`/`Fail`/`Accent`/`Command` emit 24-bit ANSI via a package-global gate set once by a new `PersistentPreRunE` on `rootCmd`. `cmd_doctor` migrates pass / warning / fail output lines through the new tokens as proof of usability. `docs/UI_PHILOSOPHY.md` gains a "Semantic color palette" section with the full table. `--json`, `--no-color`, and `NO_COLOR` contracts unchanged. (STORY-COLOR-001)
- **v0.4.42** — `docs/UI_PHILOSOPHY.md` rewritten around five named Rules (no emoji icons; small Unicode + ASCII fallback; recovery/fix consolidates into `mote doctor --fix`; prefer flags over new top-level commands; ~30 top-level commands as the discoverability ceiling) with Accept/Reject examples drawn from current code. Existing renderer invariants (closed items recede, color decision, column-alignment, `MOTE_FORCE_TTY`, `--plain`/`--pretty`/`--json` trio) preserved in an Operational details appendix. Cross-linked from `CLAUDE.md` and `AGENTS.md`. CI command-count check deferred to STORY-UIPHI-002 (STORY-UIPHI-001).
- **v0.4.41** — Customizable `PRIME.md` prose preamble with three-tier resolution (`./.memory/PRIME.md` → `<workspace>/PRIME.md` → `~/.motes/PRIME.md`); content inserted between the truncation directive and the data sections in both CLI and MCP modes; new `mote prime --export` writes a starter template to stdout (with stderr redirect hint on TTY); JSON envelope additively gains `prose_section`; 16 KiB cap with truncation marker; UTF-8 with BOM strip; tier failures fall through silently (`--debug` surfaces them). `--memories-only` and `--export` bypass override resolution. (STORY-PRIMEOVR-001)
