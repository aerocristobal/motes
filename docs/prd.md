...
**Design Note — Lens Mode supersedes self-consistency voting (Epic 17, v0.4.7):**
The original self-consistency mechanism (`batching.self_consistency_runs: N`) runs each batch prompt N times and keeps only majority-agreed visions. Lens mode is the alternative path: each of the N runs applies a distinct mental-model lens (structural, survivorship bias, feedback loops, etc.) and the reconciler synthesizes across lenses rather than voting away disagreement — visions surfaced by ≥2 lenses gain a `CrossLensAgreement` confidence signal stronger than vote count. Lens mode and `self_consistency_runs > 1` are mutually exclusive; projects enable one or the other in `.memory/config.yaml` (`batching.lens_mode.enabled`). See `docs/configuration.md` for the lens config block.
...
| **18: PRIME.md Override** | STORY-PRIMEOVR-001 | Customizable `PRIME.md` prose preamble with three-tier resolution (`./.memory/PRIME.md` → `<workspace>/PRIME.md` → `~/.motes/PRIME.md`); `mote prime --export` writes starter template; 16 KiB cap; UTF-8 with BOM strip; silent tier-failure fallthrough with `--debug` escape hatch; JSON envelope additively gains `prose_section` (v0.4.41) | Done |
| **19: UI Philosophy Codified** | STORY-UIPHI-001 | `docs/UI_PHILOSOPHY.md` rewritten around five named Rules (no emoji icons; small Unicode + ASCII fallback; recovery consolidates into `mote doctor --fix`; prefer flags over new top-level commands; ~30-command discoverability ceiling) with Accept/Reject examples drawn from current code (v0.4.42) | Done |
| **20: Semantic Color Tokens** | STORY-COLOR-001 | Six semantic tokens (`Pass`, `Warn`, `Fail`, `Accent`, `Muted`, `Command`) backed by Ayu-palette `AdaptiveColor{Light, Dark}` pairs with hand-rolled `COLORFGBG` detection; `cmd_doctor` migrates pass/warn/fail through the new tokens as proof of usability; `--json`, `--no-color`, `NO_COLOR` contracts unchanged (v0.4.43) | Done |
| **21: Two-Zone Header** | STORY-HDRZ-001 | `mote show` first line and every `mote ls` row use status icon + Accent-colored ID + ` · ` + title on the left and `[<icon> <STATUS> w<weight>]` right-aligned to terminal width on the right; closed motes mute the entire line and drop the weight segment; `--plain` uses ASCII icons and literal `  \|  ` separator; old `=== <id> ===` block and printf-table column header removed (v0.4.44) | Done |
| **22: Instruction-Doc Divergence** | STORY-DIVRG-001 | `mote doctor` reconciles shared H2 sections across `CLAUDE.md`, `AGENTS.md`, `CODEX.md`, `GEMINI.md` — configured via `.memory/config.yaml` `doctor.instruction_docs.shared_sections`; `--verbose` prints per-section status; `--fix` copies authoritative section into diverged peers (idempotent); per-section opt-out via `<!-- mote-doctor-divergence: ok -->` HTML marker (v0.4.45) | Done |
| **23: Landing the Plane** | STORY-LAND-001 | `CLAUDE.md` `## Landing the Plane (Session Completion)` codified as eight ordered steps with `### ⚠ Anti-patterns` callout, `### Hand-off follow-up prompt` template, and `### Example session` fenced bash block; `scripts/check-landing-the-plane.sh` CI contract test; same section propagated verbatim into `AGENTS.md`, `CODEX.md`, `GEMINI.md` for STORY-DIVRG-001 reconciliation (v0.4.46) | Done |
| **24: Canonical Test Wrapper** | STORY-TWRAP-001 | `bash scripts/test.sh` wraps `go test` with `.test-skip` (mandatory `# mote-<id>` rationale — anything else exits 2 before `go test` runs), default 5m timeout (override via `TEST_TIMEOUT`), `TEST_VERBOSE=1`/`TEST_RUN=TestX` mapping; post-run pass enumerates real tests via `go test -list` and warns on stale skip entries without changing exit code; `Makefile`'s `test:` target delegates; `wrapper-tests` CI job runs 13 contract tests via a stubbed `go` binary on `$PATH`; agent docs (CLAUDE/AGENTS/CODEX/GEMINI) instruct the wrapper (v0.4.47) | Done |
