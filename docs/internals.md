...
## Doc-flag freshness contract

The `doc-flags` CI job (STORY-DOCFLAGS-001) builds `mote` and runs `./mote check-doc-flags`. The check scans every doc file in scope for `mote <command> --<flag>` references and fails when any reference does not resolve against the live Cobra command tree. A `cmd/mote/cli_surface.go` walker introspects the in-process command tree directly — there is no JSON artifact and no help-text parsing, so the check is immune to layout drift in Cobra's help renderer.

**Scope.** README.md, AGENTS.md, CLAUDE.md, CODEX.md, GEMINI.md, and every `docs/*.md` except `docs/example-*-config.md` (those files are configuration samples, not flag examples). Override with `--paths a.md,b.md` for ad-hoc scans.

**Resolution.** For each `(command, flag)` reference: (1) accept if the resolved command lists the flag locally; (2) accept if the flag is a root-level persistent flag (`--no-color`, `--plain`, `--pretty`, `--help`, `--version`); (3) accept if the flag is in `scripts/removed-flags.txt` AND the reference is allowlisted (nearest H2 heading text OR the enclosing paragraph case-insensitively contains one of `changelog`, `removed`, `deprecated`, `historical`); (4) otherwise fail. The scanner does longest-prefix matching to disambiguate `mote add foo bar --type=task` (resolves to `add`) from `mote compliance export --format=oscal-cdef` (resolves to `compliance export`).

**Removed-flag manifest.** When a flag is renamed or retired from the live tree, add an entry to `scripts/removed-flags.txt` in the format `<command path> <--flag>` (one or two words for the command path; lines starting with `#` are comments). Without an entry, references to the gone flag fail as "unknown flag for command" instead of the more accurate "removed flag, not in allowlisted context".

**Per-reference suppression.** When a doc legitimately needs to mention a flag the check would otherwise reject — a future flag not yet implemented, a string-literal example, an asymmetric reference — prefix the line with the HTML comment `<!-- doc-flags: ignore-next -->`. The directive suppresses the next non-blank line; if that line opens a fenced code block, the entire block is suppressed. The marker is self-documenting (the tool name is in the directive) so a future reader can grep for it and understand the blast radius.
